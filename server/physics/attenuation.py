...
def mass_attenuation_coefficient(
    ...
    # Try NASA data first
    attenuation_data = _get_attenuation_table()
    if attenuation_data is not None and material_symbol in attenuation_data.get("materials", {}):
        energies = attenuation_data["photon_energy_mev"]
        values = attenuation_data["materials"][material_symbol]
...
def _get_attenuation_table() -> dict[str, Any] | None:
    """Return the mass_attenuation sub-table from NASA data, or None."""
    data = get_nasa_data()
    return data.get("mass_attenuation") if data else None
